...
struct MDRequestImpl : public MutationImpl {
  ...
  struct Params {
    ...
    Params() : attempt(0), client_req(NULL),
        triggering_slave_req(NULL), slave_to(-1) {}
  };
  MDRequestImpl(const Params& params) :
    ...
    slave_request(NULL), internal_op(-1), retry(0),
    waited_for_osdmap(false), _more(NULL) {
    in[0] = in[1] = NULL;
    // we don't save the triggering request
    // we just want it to set up the start times
  }
  ~MDRequestImpl();
